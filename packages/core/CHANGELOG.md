# @clipr/core

## 0.0.17

### Patch Changes

- build(deps-dev): bump the dev-dependencies group with 6 updates (#45)

## 0.0.16

### Patch Changes

- build(deps): bump hono from 4.12.12 to 4.12.14 (#35)

## 0.0.15

### Patch Changes

- build(deps): bump hono from 4.12.10 to 4.12.12 (#27)

## 0.0.14

### Patch Changes

- fix: update vite to address CVE-2026-39363 (#25)

## 0.0.13

### Patch Changes

- build(deps-dev): bump the dev-dependencies group with 3 updates
- build(deps): bump pnpm/action-setup from 4 to 5 (#20)

## 0.0.12

### Patch Changes

- fix: bump defu to >=6.1.5 (GHSA-737v-mqg7-c878 prototype pollution) (#17)

## 0.0.11

### Patch Changes

- feat: theme-based hero synonyms, terminal polish, npm stats, logo, marquee

## 0.0.10

### Patch Changes

- feat: animated SVG logo + enhanced marquee with tech logos

## 0.0.9

### Patch Changes

- fix: update shorten test for public endpoint, lint fixes

## 0.0.8

### Patch Changes

- fix: wrap shorten script in DOMContentLoaded — elements not available in head

## 0.0.7

### Patch Changes

- chore(release): publish v0.0.6 — fix workspace:\* in npm tarball

## 0.0.5

### Patch Changes

- fix: use GITHUB_TOKEN for changesets PR creation, PAT for checkout

## 0.0.4

### Patch Changes

- fix: skip auto-changeset on version bump commits, fix npm package name in homebrew

## 0.0.3

### Patch Changes

- fix: rename CLI package to clipr-cli (clipr taken on npm)

## 0.0.2

### Patch Changes

- fix: release workflow heredoc syntax for auto-changeset
